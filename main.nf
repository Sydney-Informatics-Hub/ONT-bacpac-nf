...
workflow {
  ...
  // If none of the above are a problem, then run the workflow
    ...
    if (params.input_directory){
      ...
      samplesheet = channel.fromPath(params.samplesheet, checkIfExists: true).first()
      ...
      sequencing_summary = samplesheet
        ...
        .map { batch, summary_list -> {
          ...
          assert filtered_summary_list.size() != 0 : "Error: No sequencing summary file provided for batch '${batch}'."
      ...
      pre_unzipped_fq_dirs = samplesheet
        .filter { _barcode, _sample, _batch, f -> f.isDirectory() }
    }
